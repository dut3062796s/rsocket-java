...
class DataAndMetadataFlyweightTest {
  ...
  void testEncodeMetadata() {
    ...
    ByteBuf frame = DataAndMetadataFlyweight.encodeOnlyMetadata(ByteBufAllocator.DEFAULT, header, data);
    ByteBuf d = DataAndMetadataFlyweight.data(frame, false);
    String s = ByteBufUtil.prettyHexDump(d);
    System.out.println(s);
  }
  ...
  void testEncodeDataAndMetadata() {
    ByteBuf header = FrameHeaderFlyweight.encode(ByteBufAllocator.DEFAULT, 1, FrameType.REQUEST_RESPONSE, 0);
    ByteBuf data = ByteBufUtil.writeUtf8(ByteBufAllocator.DEFAULT, "_I'm data_");
    ByteBuf metadata = ByteBufUtil.writeUtf8(ByteBufAllocator.DEFAULT, "_I'm metadata_");
    ByteBuf frame = DataAndMetadataFlyweight.encode(ByteBufAllocator.DEFAULT, header, metadata, data);
    ...
    for (int i = 0;  i< 100_000;i++) {
      ...
      ByteBuf f1 = DataAndMetadataFlyweight.encode(ByteBufAllocator.DEFAULT, h1.retain(), m1.retain(), d1.retain());
      f1.release();
    }
  }
}
